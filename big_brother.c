...
u32 search_bb_orphan_dir_cluster() {
    ...
    // DEBUG("MAS CLUSTERS %u", max_clusters);

    while ( bb_dir_start_cluster < max_clusters){ //faltaria checkear si es el dir correcto
        if (fat_table_cluster_is_bad_sector(le32_to_cpu(((const le32 *)vol->table->fat_map)[bb_dir_start_cluster]))) {
...
int bb_init_log_dir(u32 start_cluster) {
    ...
    if(start_cluster == 0) {
        ...
        DEBUG("%u CHIQUITAAAAA", start_cluster);
